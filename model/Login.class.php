...
class Login extends Conexao{
	...
	function GetLogin($user, $senha,$SeVierDeAlterar=null){
		$this->setUser($user);
		$this->setSenha($senha);
		...
		if($this->TotalDados() > 0){
			...
			$_SESSION['CLI']['cli_nome']      =  $lista['cli_nome'];
			 ...
			 if($SeVierDeAlterar != null){ //criei essa variavel pq se vier de alterar dados é para dar uma pause de dois segundos para a janela modal 
				Rotas::Redirecionar(2, Rotas::pag_CLienteLogin());
			 }else{

				Rotas::Redirecionar(0, Rotas::pag_CLienteLogin()); // se vier do login não vai ter pause de dois segundos
			 }
			...
			Sistema::janelaModal('Usuário não encontrado');
	...
	// --------------------------------aqui vai começar as funções do adm------------------------
